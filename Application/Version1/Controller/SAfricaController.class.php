...
//        echo $data;
    ...
    public function register(){
        ...
        if ($code == $re_code){
            ...
            if(!empty($login)){
                ...
                $con['nickname'] = substr($con['mobile'],0,9).'**';
                $con['registertime'] = time();
                $insert = $model_user->add($con);
                $data['user_id'] = $insert;
                $first = I('post.first_stream','');
                if(''!=$first){
                    $ins['first_stream'] = I('post.first_stream','');
                    $ins['second_stream'] = I('post.second_stream','');
                    $ins['user_id'] = $insert;
                    $stream = M('stream');
                    $insert = $stream->add($ins);
                }
    ...
    public function login(){
        ...
        $options = array('client_id' => 'YXA6ryLaYB6qEee9ag-MCEtXOA',
            ...
            'app_name' => 'yanku');
        $token = new HXController($options);
        $token = $token->deleteUser($user_id);
...
//        $redis->select(5);
        ...
        if (is_int($user_id)){
            ...
            $insert = 'yk_'.$time;
            $result = $token->createUser("$insert",'yanku321');
            empty($result)?$this->json_rest(0,array('user_id'=>'-1')):$res['user_id']=$result['entities'][0]['username'];
        }else{
            $res['attu'] = 0;
            $res['user_id'] = $user_id;
        }
        $res['nickname'] = $key['nickname'];
        $res['user_img'] = $key['user_img'];
        $res['praise'] = $key['praise'];
        $res['count'] = $key['count'];
