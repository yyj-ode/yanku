...
//------------------------------------------------------用户体系
    ...
    public function __construct($options) {
        ...
        if (! empty ( $this->org_name ) && ! empty ( $this->app_name )) {
            $this->url = 'http://a1.easemob.com/' . $this->org_name . '/' . $this->app_name . '/';
        }
    }
    /**
     *获取token
    ...
    */
     ...
     * 视频直播间地址
     */
    function liverooms($options){
        $url=$this->url.'liverooms?status=ongoing';
//        echo $url;
//        die();
        $header=array($this->getToken());
        $body=json_encode($options);
     ...
     * 视频直播间流
     */
    function setLiveroomsstream($options){
     ...
     * 设置直播间主播
    */
    function setChatroomsuper($options){
