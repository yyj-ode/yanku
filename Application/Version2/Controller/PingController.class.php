...
class PingController extends ResController
{
   ...
   public function kubi_rmb($amount){
       switch ($amount){
           case 12:
               $amount = $amount*66+20;
               break;
           case 30:
               ...
               break;
           case 60:
               $amount = $amount*66+120;
               break;
           case 108:
               $amount = $amount*66+250;
               break;
           case 308:
               $amount = $amount*66+720;
               break;
           case 618:
               $amount = $amount*66+1500;
               break;
           case 998:
               $amount = $amount*66+2500;
               break;
           case 5898:
               $amount = $amount*66+33333;
               break;
           default :
               $amount = $amount*66;
               break;
   ...
   public function pay(){
       ...
       $ping = \Pingpp\Charge::create(array(
            'order_no'  => $order_no,
            'amount'    => $amount,
            'app'       => array('id' => 'app_1Cy5WT8mfXf1arvf'),
            'channel'   => $channel,
       ...
       $amount = $amount/100;
       $kubi = $this->kubi_rmb($amount);
       $rec = M('recharge');
       $res = $rec->where("user_id=$user_id")->find();
       if (!$res){
           $my = M('user');
           $my->setInc('kubi',200);
       }
       ...
       if ($insert){
           $data['pay'] = json_decode($ping,true);
           $data['kubi'] = $kubi;
