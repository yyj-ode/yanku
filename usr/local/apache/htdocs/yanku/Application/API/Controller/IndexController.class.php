...
class IndexController extends ResController {
    ...
    //心跳开启标识
    private $onlive = 0;
    private $sleep_time = 15;//多长时间执行一次
    /*
         * 忘记密码
         * param1：mobile手机号
         * param2：pwd密码
    ...
    */
    ...
    public function register(){
        ...
        if(!empty($login)){
            ...
            empty($result)?$this->json_rest(0):$this->json_rest(1,$data);
        }
    }
    /*
     * 注册详细信息
    ...
    public function schedule_type_info(){
        ...
        $join = $join->field('user_id,schedule_status')->where("user_id=$user_id")->find();
        isset($join['user_id'])?$data['info']['join']=1:$data['info']['join']=0;
        if ($data['info']['join']==1){
            if ($join['schedule_status']==2 && date("Y-m-d") <= date('Y-m-d', strtotime("+1 week", $data['acttime']))){
                $data['info']['show'] = 1;
            }else{
                $data['info']['show'] = 0;
            }
    ...
    public function index_yk(){
        ...
        $data['list'] = $Index->field("yk_user.user_id,group_concat(yk_user_type.type) as type,yk_user.nickname,yk_user.user_img,yk_user.nameaudit")
                                                 ...
                                                 ->where("yk_user_type.type!=0")
                                                 ->group("yk_user.user_id")
                                                 ->order('RAND()')
                                                 ->limit($start,20)
                                                 ->select();
        $data['banner'] = $this->img_urls($data['banner'],'img','User');
    ...
    public function yk_type(){
        ...
        $audit = $audit==1?"AND yk_user.nameaudit=1":"AND yk_yanyiaudit.status=2";
        switch ($order){
            case 0:
                $data['list'] = $typeInfo->field("yk_user.user_id,group_concat(yk_user_type.type) as type,yk_user.nickname,yk_user.user_img,yk_user.nameaudit")
                    ...
                    ->where("yk_user_type.type!=0 $audit $sex")
                    ->group("yk_user.user_id")
                    ->order('RAND()')
                    ->limit($start,20)
                    ->select();
...
//        $start = I('get.start',0);
//        $city = I('get.city_id');
//        $data = $typeInfo->field('user_id,nickname,user_img,type_cont,COUNT(a.attu_id) as cont,nameaudit')
//                         ->group('attu_id')
//                         ->order('cont DESC')
//                         ->select();
...
//        $this->token_audit($user_id,$token);
        ...
        if (isset($user_id)){
            ...
            if ($data['user_img'] != ''){
//                $data['user_img'] = 'http://' . $_SERVER ['HTTP_HOST'] . '/Upload/'  . $data['user_img'];
                $data = $this->img_url($data,'user_img');
            }else{
                $this->json_rest(0);
                die();
            }
...
//            ->select();
    ...
    public function join_schedule_type(){
        ...
        $data = $work->field('ifnull(yk_user_schedule.schedule_id,"") as schedule_id,yk_schedule.acttime,yk_schedule.schedule_title,yk_schedule.schedule_img,yk_schedule.schedule_type,yk_user.nickname')
            ->join('LEFT JOIN yk_schedule ON yk_user_schedule.schedule_id=yk_schedule.schedule_id LEFT JOIN yk_user ON yk_user_schedule.user_id=yk_user.user_id')
            ->where($where)
            ->limit($start,5)
            ->select();
...
//        $data = $this->imgs_url($data,'work_img','User');
    ...
    public function delete_card(){
        ...
        $data[$delete_type."_id"] = $card_id;
        $type = M($delete_type);
        $type_img = M($delete_type.'_img');
        $data = $type_img->where($data)->select();
        $type->startTrans();//开启事务
        $result = $type->where($con)->delete();
        $result1 = $type_img->where($data)->delete();
        if ($result&&$result1){
            $type->commit();
            $type_img->commit();
            foreach ($data as $k=>&$v){
    ...
    public function mine_data(){
        ...
        $this->token_audit($user_id,$token);
        $User = M('user');
        $data= $User->field('yk_user.user_img,yk_user.level,yk_user.acquirement,yk_user.user_id,yk_user.nickname,yk_user.sex,yk_user.city as user_city,yk_user.kubi,yk_user.birthday,yk_user.height as user_height,yk_user.weight,yk_user.threedimensional,yk_user.signature,group_concat(distinct yk_user_type.type) as tag')
            ->join('LEFT join yk_user_type on yk_user.user_id=yk_user_type.user_id')
...
//                $this->error($upload->getError());
            ...
            $console['create_time'] = strtotime("now");
            $user = M('user');
            $audit = M('nameaudit');
...
//                        die();
                        ...
                        $redis->select(3);
                        $redis->SREM($key['channel_type'],$key['user_id']);
                        $key2 = $redis->select(2);
                        $key = $redis->delete($v);
                    }
                }
...
//                            $room->where("user_id=$k")->save($data);
//                            $history = M('live_history');
//                            $count = $history->add($key);
//                            $redis->select(2);
//                            $result = json_decode($redis->get($k), true);
//                            $options = array('client_id' => 'YXA6ryLaYB6qEee9ag-MCEtXOA',
//                                'client_secret' => 'YXA6P275ejRYIcHpCHj_eqtonJUmJb4',
//                                'org_name' => '1134170411178481',
//                                'app_name' => 'yanku');
...
//            }
    ...
    public function myaccount(){
        ...
        $this->token_audit($user_id,$token);
        $user = M('user');
        $data = $user->field('kubi,kudou')->where("user_id=$user_id")->find();
        $this->json_rest(1,$data);
    }
